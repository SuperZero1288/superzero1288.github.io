(() => {
  ...
  const styles=[
    {id:'style-default',name:'Default',label:'DEFAULT INTERFACE',light:true},
    {id:'style-material',name:'Material',label:'MATERIAL / GOOGLE',light:true},
    {id:'style-liquidglass',name:'LiquidGlass',label:'LIQUID GLASS / APPLE',light:true},
    {id:'style-fluent',name:'Fluent Design',label:'FLUENT / MICROSOFT',light:true},
    {id:'style-vrchat',name:'VRChat UI',label:'VRCHAT HUD / ERROR',light:false},
    {id:'style-unity',name:'Unity Engine',label:'UNITY ENGINE / MISSING ASSET',light:false}
  ];
    ...
    <div class="floating-status-bar page-floating-bar">
      ...
      <div class="status-capsule time-capsule" id="pageTimeCapsule" title="クリックで12h/24h切り替え（連打で...？）">
        ...
        <div class="calendar-popup" id="pageCalendarPopup">
          <div class="calendar-header" id="pageCalendarHeader"></div>
          <div class="calendar-weekdays"><span class="sun">日</span><span>月</span><span>火</span><span>水</span><span>木</span><span>金</span><span class="sat">土</span></div>
          <div class="calendar-days-grid" id="pageCalendarDays"></div>
        </div>
      </div>
  ...
  const timeCapsule=document.getElementById('pageTimeCapsule');
  const calendarPopup=document.getElementById('pageCalendarPopup');
  const calendarHeader=document.getElementById('pageCalendarHeader');
  const calendarDays=document.getElementById('pageCalendarDays');
  ...
  const restoreAppearance=()=>{
    try{
      const saved=JSON.parse(localStorage.getItem('zero-site-appearance')||'{}');
      const savedIndex=styles.findIndex((style)=>style.id===saved.style);
      if(savedIndex>=0)styleIndex=savedIndex;
      document.body.classList.remove(...styles.map((style)=>style.id),'light-theme');
      document.body.classList.add(styles[styleIndex].id);
      if(saved.light&&styles[styleIndex].light)document.body.classList.add('light-theme');
    }catch(error){console.warn('表示設定を復元できませんでした',error);}
  };
  ...
  const renderCalendar=(now)=>{
    ...
    calendarHeader.textContent=`${year}.${String(month+1).padStart(2,'0')}`;
    ...
    for(let day=1;day<=new Date(year,month+1,0).getDate();day++){
      ...
      cell.addEventListener('click',(event)=>{
        ...
        navigator.clipboard.writeText(value).then(()=>showToast(`コピーしました: ${value}`)).catch(()=>showToast(value));
      });
  ...
  const updateTime=()=>{
    const now=new Date();
    if(isThursdayMode){time.textContent=now.getDay()===4?'はい':'いいえ';return;}
    let hours=now.getHours();
    const minutes=String(now.getMinutes()).padStart(2,'0');
    if(is24Hour)time.textContent=`${String(hours).padStart(2,'0')}:${minutes}`;
    else{const ampm=hours>=12?'PM':'AM';hours=hours%12||12;time.textContent=`${ampm} ${hours}:${minutes}`;}
    if(lastRenderedMonth!==now.getMonth()){renderCalendar(now);lastRenderedMonth=now.getMonth();}
  };
  ...
  const updateThemeButton=()=>{
    ...
    document.querySelectorAll('[data-style-label]').forEach((element)=>element.textContent=styles[styleIndex].label);
  };
  ...
  timeCapsule.addEventListener('click',()=>{
    clickCount+=1;
    if(clickCount>=5){
      if(Math.random()<.3){isThursdayMode=!isThursdayMode;showToast(isThursdayMode?'Is it Thursday...?':'時刻表示に戻りました');}
      else is24Hour=!is24Hour;
      clickCount=0;
    ...
    if(!isThursdayMode)is24Hour=!is24Hour;
    updateTime();
  });
